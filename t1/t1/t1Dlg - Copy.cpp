...
*/
	...
	editorMy.SetWindowTextW(_T("Hello world..."));
...
BOOL Ct1Dlg::PreTranslateMessage(MSG* pMsg)
{
    // RT display info.
    {
		...
		wchar_t systemTime[150];
		swprintf_s(systemTime, sizeof(systemTime) / sizeof(wchar_t), L"%4d/%02d/%02d %02d:%02d:%02d.%03d 星期%1d 当前价格%d\n",
            sys_time.wYear,
            sys_time.wMonth,
            sys_time.wDay,
            sys_time.wHour,
        ...
        wchar_t coordinates[30];
		swprintf_s(coordinates, sizeof(coordinates) / sizeof(wchar_t), L"%d, %d", RELATIVE_HORIZON_DISTANCE, RELATIVE_VERTICAL_DISTANCE);
        //sprintf(coordinates, "%d, %d", RELATIVE_HORIZON_DISTANCE, RELATIVE_VERTICAL_DISTANCE);
        infoPanelEditor.SetWindowTextW(coordinates);
    }
...
bool Ct1Dlg::manageUserEvent(MSG* pMsg)
{
    ...
    if (pMsg->message == WM_KEYDOWN)
    {
        ...
        else if (pMsg->wParam == VK_F4)
        {
            ...
            //display result
            wchar_t coordinates[60];
			swprintf_s(coordinates, sizeof(coordinates) / sizeof(wchar_t), 
				       L"%s, %s", 
				       captureText(RELATIVE_LEFT, RELATIVE_TOP, RELATIVE_RIGHT, RELATIVE_BOTTOM).c_str(),
                       price.c_str());
            editorMy.SetWindowTextW(coordinates);
